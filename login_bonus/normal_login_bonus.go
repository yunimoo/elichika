...
func latestLoginBonusTime(timePoint time.Time) time.Time {
	...
	return time.Date(year, month, day, 0, 0, *config.Conf.LoginBonusSecond, 0, timePoint.Location())
}
func NextLoginBonusTime(timePoint time.Time) time.Time {
	year, month, day := timePoint.Date()
	return time.Date(year, month, day+1, 0, 0, *config.Conf.LoginBonusSecond, 0, timePoint.Location())
}
